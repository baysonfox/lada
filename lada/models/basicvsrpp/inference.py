...
def load_model(
    config: str | dict | None, checkpoint_path, device, fp16=False, compile_model=False
) -> BasicVSRPlusPlusGan | BasicVSR:
    ...
    if config is None:
        ...
        raise Exception(
            "unsupported value for 'config', Must be either a file path to a config file or a dict definition of the model"
        )
    model = MODELS.build(config)
    assert isinstance(model, BasicVSRPlusPlusGan) or isinstance(model, BasicVSR), (
        "Unknown model config. Must be either stage1 (BasicVSR) or stage2 (BasicVSRPlusPlusGan)"
    )
    ...
    if fp16:
        model = model.half()
    if compile_model:
        logger.info("Compiling restoration model with torch.compile ...")
        model.generator = torch.compile(model.generator)
        if hasattr(model, "generator_ema") and model.generator_ema is not None:
            model.generator_ema = torch.compile(model.generator_ema)
    return model
...
def inference(
    ...
    with torch.no_grad():
        input = torch.stack(
            image_utils.img2tensor(video, bgr2rgb=False, float32=True), dim=0
        )
        ...
        output = image_utils.tensor2img(
            result, rgb2bgr=False, out_type=np.uint8, min_max=(0, 1)
        )
