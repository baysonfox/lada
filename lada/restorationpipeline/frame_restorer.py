...
class FrameRestorer:
    def __init__(
        ...
        max_clips_in_mosaic_clips_queue = max(
            1, (512 * 1024 * 1024) // (self.max_clip_length * 256 * 256 * 4)
        )  # 4 = 3 color channels + mask
        self.mosaic_clip_queue = PipelineQueue(
            name="mosaic_clip_queue", maxsize=max_clips_in_mosaic_clips_queue
        )
        ...
        self.restored_clip_queue = PipelineQueue(
            name="restored_clip_queue", maxsize=max_clips_in_restored_clips_queue
        )

        # double-buffer: 1 clip being restored on GPU + 1 pre-prepared and ready
        self.prepared_clip_queue = PipelineQueue(name="prepared_clip_queue", maxsize=2)
        ...
        self.clip_preparation_thread: PipelineThread | None = None
    ...
    def start(self, start_ns=0):
        with self.start_stop_lock:
            assert (
                ...
                and self.clip_preparation_thread is None
            ), (
                "Illegal State: Tried to start FrameRestorer when it's already running. You need to stop it first"
            )
            ...
            self.clip_preparation_thread = PipelineThread(
                name="clip preparation worker",
                target=self._clip_preparation_worker,
                error_handler=self._on_worker_thread_error,
            )
            ...
            self.clip_preparation_thread.start()
    ...
    def stop(self):
        ...
        with self.start_stop_lock:
            ...
            # --- stop clip_preparation_worker ---
            # unblock consumer (reads from mosaic_clip_queue)
            threading_utils.put_queue_stop_marker(self.mosaic_clip_queue)
            # unblock producer (writes to prepared_clip_queue)
            threading_utils.empty_out_queue(self.prepared_clip_queue)
            if self.clip_preparation_thread:
                self.clip_preparation_thread.join()
                logger.debug("FrameRestorer: joined clip_preparation_thread")
            self.clip_preparation_thread = None

            # --- stop clip_restoration_worker ---
            # unblock consumer (reads from prepared_clip_queue)
            threading_utils.put_queue_stop_marker(self.prepared_clip_queue)
            # unblock producer (writes to restored_clip_queue)
    ...
    def _restore_frame(
        self, frame: ImageTensor, frame_num: int, restored_clips: list[Clip]
    ):
        """
        ...
        target_dtype = (
            torch.float32 if is_cpu_input else self.mosaic_restoration_model.dtype
        )
        ...
        def _blend_gpu(
            ...
            temp = clip_img.to(
                dtype=self.mosaic_restoration_model.dtype, device=frame_roi.device
            )
            temp.sub_(roi_f)
    ...
    def _clip_buffer_contains_all_cips_needed_for_current_restoration(
        self, current_frame_num, num_mosaic_detections, clip_buffer
    ):
        num_clips_starting_at_frame = len(
            [clip for clip in clip_buffer if clip.frame_start == current_frame_num]
        )
        assert num_clips_starting_at_frame <= num_mosaic_detections
        return num_clips_starting_at_frame == num_mosaic_detections

    def _clip_preparation_worker(self):
        logger.debug("clip preparation worker: started")
        eof = False
        while not (eof or self.stop_requested):
            clip = self.mosaic_clip_queue.get()
            if self.stop_requested or clip is STOP_MARKER:
                logger.debug(
                    "clip preparation worker: mosaic_clip_queue consumer unblocked"
                )
                break
            if clip is EOF_MARKER:
                eof = True
                self.prepared_clip_queue.put(EOF_MARKER)
                if self.stop_requested:
                    logger.debug(
                        "clip preparation worker: prepared_clip_queue producer unblocked"
                    )
                    break
            else:
                prepared = None
                if (
                    ...
                    prepared = self.mosaic_restoration_model.prepare(clip.frames)
                self.prepared_clip_queue.put((clip, prepared))
                if self.stop_requested:
                    logger.debug(
                        "clip preparation worker: prepared_clip_queue producer unblocked"
                    )
                    break
        if eof:
            logger.debug("clip preparation worker: stopped itself, EOF")
        else:
            logger.debug("clip preparation worker: stopped by request")

    def _clip_restoration_worker(self):
        logger.debug("clip restoration worker: started")
        eof = False
        while not (eof or self.stop_requested):
            item = self.prepared_clip_queue.get()
            if self.stop_requested or item is STOP_MARKER:
                logger.debug(
                    "clip restoration worker: prepared_clip_queue consumer unblocked"
                )
                break
            if item is EOF_MARKER:
                ...
                if prepared is not None:
                    restored_clip_images = (
                        self.mosaic_restoration_model.restore_prepared(prepared)
                    )
                    assert len(restored_clip_images) == len(clip.frames)
                    for i in range(len(restored_clip_images)):
                        assert clip.frames[i].shape == restored_clip_images[i].shape
    ...
    def _read_next_frame(
    ...
    ) -> tuple[int, np.ndarray, int] | StopMarker | EofMarker:
        try:
            ...
            if self.stop_requested or elem is STOP_MARKER:
                logger.debug(
                    "frame restoration worker: frame_detection_queue consumer unblocked"
                )
                return STOP_MARKER
            assert elem is EOF_MARKER, (
                f"Illegal state: Expected to read EOF_MARKER from detection queue but received f{elem}"
            )
            return EOF_MARKER
        elem = self.frame_detection_queue.get()
        if self.stop_requested or elem is STOP_MARKER:
            ...
            return STOP_MARKER
        assert elem is not EOF_MARKER and elem is not STOP_MARKER, (
        ...
        assert detection_frame_num == expected_frame_num, (
            f"frame detection queue out of sync: received {detection_frame_num} expected {expected_frame_num}"
        )
    ...
    def _read_next_clip(
        ...
        if self.stop_requested or clip is STOP_MARKER:
            logger.debug(
                "frame restoration worker: restored_clip_queue consumer unblocked"
            )
            return STOP_MARKER
        if clip is EOF_MARKER:
            return EOF_MARKER
        ...
        with video_utils.VideoReader(self.video_meta_data.video_file) as video_reader:
            ...
            while not (self.eof or self.stop_requested):
                ...
                if num_mosaics_detected > 0:
                    while (
                        queue_marker is None
                        and not self._clip_buffer_contains_all_cips_needed_for_current_restoration(
                            frame_num, num_mosaics_detected, clip_buffer
                        )
                    ):
