...
try:
    ...
    if IS_FLATPAK:
        ...
        print(
            _(
                "In order to use the application you need to install one of Lada's Flatpak Add-Ons that is compatible with your hardware"
            )
        )
        sys.exit(1)
    else:
...
def setup_argparser() -> argparse.ArgumentParser:
    ...
    example2_text = _(
        "Restore all videos found in the specified directory and save them to a different folder:"
    )
    example2_command = _(
    ...
    example3_command = _(
        "%(prog)s --input input.mp4 --encoding-preset hevc-nvidia-gpu-hq"
    )

    example4_text = _(
        "Set encoding parameters directly without using an encoding preset:"
    )
    example4_command = _(
        "%(prog)s --input input.mp4 --encoder libx265 --encoder-options '-crf 26 -preset fast -x265-params log_level=error'"
    )
    ...
    group_general.add_argument(
        "--input",
        type=str,
        help=_("Path to pixelated video file or directory containing video files"),
    )
    group_general.add_argument(
        "--output",
        type=str,
        help=_(
            "Path used to save output file(s). If path is a directory then file name will be chosen automatically (see --output-file-pattern). If no output path was given then the directory of the input file will be used"
        ),
    )
    group_general.add_argument(
        "--temporary-directory",
        ...
        help=_(
            "Directory for temporary video files during restoration process. Alternatively, you can use the environment variable TMPDIR. (default: %(default)s)"
        ),
    )
    group_general.add_argument(
        "--output-file-pattern",
        type=str,
        default="{orig_file_name}.restored.mp4",
        help=_(
            "Pattern used to determine output file name(s). Used when input is a directory, or a file but no output path was specified. Must include the placeholder '{orig_file_name}'. (default: %(default)s)"
        ),
    )
    group_general.add_argument(
        "--device",
        type=str,
        default=get_default_torch_device(),
        help=_(
            'Device used for running Restoration and Detection models. Use "--list-devices" to see what\'s available (default: %(default)s)'
        ),
    )
    group_general.add_argument(
        "--fp16",
        ...
        help=_(
            "Reduces VRAM usage and may increase speed on modern GPUs, with negligible quality difference. (default: %(default)s)"
        ),
    )
    group_general.add_argument(
        "--list-devices", action="store_true", help=_("List available devices and exit")
    )
    group_general.add_argument(
        "--version", action="store_true", help=_("Display version and exit")
    )
    group_general.add_argument(
    ...
    export.add_argument(
        ...
        help=_(
            'Select encoding preset by name. Use "--list-encoding-presets" to see what\'s available. Ignored if "--encoder" and "--encoder-options" are used (default: %(default)s)'
        ),
    )
    export.add_argument(
        "--list-encoding-presets",
        action="store_true",
        help=_("List available encoding presets and exit"),
    )
    export.add_argument(
        "--encoder",
        type=str,
        help=_(
            'Select video encoder by name. Use "--list-encoders" to see what\'s available. (default: %(default)s)'
        ),
    )
    export.add_argument(
        "--list-encoders",
        action="store_true",
        help=_("List available encoders and exit"),
    )
    export.add_argument(
        "--encoder-options",
        type=str,
        help=_(
            'Space-separated list of options for the encoder set via "--encoder". Use "--list-encoder-options" to see what\'s available. (default: %(default)s)'
        ),
    )
    export.add_argument(
        ...
        metavar="ENCODER",
        type=str,
        help=_("List available options of the given encoder and exit"),
    )
    export.add_argument(
        "--mp4-fast-start",
        default=False,
        action=argparse.BooleanOptionalAction,
        help=_(
            'Allows playing the file while it\'s being written. Sets .mp4 mov flags "frag_keyframe+empty_moov+faststart". (default: %(default)s)'
        ),
    )

    group_restoration = parser.add_argument_group(_("Mosaic Restoration"))
    group_restoration.add_argument(
        "--list-mosaic-restoration-models",
        action="store_true",
        help=_(
            "List available restoration models found in model weights directory and exit (default location is './model_weights' if not overwritten by environment variable LADA_MODEL_WEIGHTS_DIR)"
        ),
    )
    group_restoration.add_argument(
        ...
        help=_(
            'Name of detection model or path to model weights file. Use "--list-mosaic-restoration-models" to see what\'s available. (default: %(default)s)'
        ),
    )
    group_restoration.add_argument(
        ...
        default=None,
        help=_(
            "Path to restoration model configuration file. You'll not have to set this unless you're training your own custom models"
        ),
    )
    group_restoration.add_argument(
        "--max-clip-length",
        ...
        default=180,
        help=_(
            "Maximum number of frames for restoration. Higher values improve temporal stability. Lower values reduce memory footprint. If set too low flickering could appear (default: %(default)s)"
        ),
    )
    ...
    group_detection.add_argument(
        ...
        default="v4-fast",
        help=_(
            'Name of detection model or path to model weights file. Use "--list-mosaic-detection-models" to see what\'s available. (default: %(default)s)'
        ),
    )
    group_detection.add_argument(
        "--list-mosaic-detection-models",
        action="store_true",
        help=_(
            "List available detection models found in model weights directory and exit (default location is './model_weights' if not overwritten by environment variable LADA_MODEL_WEIGHTS_DIR)"
        ),
    )
    group_detection.add_argument(
        "--detect-face-mosaics",
        ...
        default=False,
        help=_(
            "Detect and ignore areas of pixelated faces. Can prevent restoration artifacts but may worsen detection of NSFW mosaics. Available for models v3 and newer. (default: %(default)s)"
        ),
    )
...
def process_video_file(
    ...
    def _video_writer_worker(video_writer):
        ...
        try:
            while True:
                ...
                if item is None:
                    break
                frame, pts = item
                video_writer.write(frame, pts, bgr2rgb=True)
    ...
    try:
        ...
        with VideoWriter(
            ...
            writer_thread = threading.Thread(
                target=_video_writer_worker, args=(video_writer,), daemon=True
            )
            writer_thread.start()
            ...
            for elem in frame_restorer:
                ...
                if writer_error is not None:
                    success = False
                    frame_restorer_progressbar.error = True
                    print("Error on export:", writer_error)
                    break
                (restored_frame, restored_frame_pts) = elem
                write_queue.put((restored_frame, restored_frame_pts))
    ...
    if success:
        ...
        audio_utils.combine_audio_video_files(
            video_metadata, video_tmp_file_output_path, output_path
        )
    else:
...
def main():
    ...
    if args.device.startswith("cuda") and not torch.cuda.is_available():
        print(
            _("GPU {device} selected but CUDA is not available").format(
                device=args.device
            )
        )
        sys.exit(1)
    if args.device == "mps":
        ...
        if not has_mps():
            ...
            sys.exit(1)
    if (
        "{orig_file_name}" not in args.output_file_pattern
        or "." not in args.output_file_pattern
    ):
        print(
            _(
                "Invalid file name pattern. It must include the template string '{orig_file_name}' and a file extension"
            )
        )
        sys.exit(1)
    if (
        ...
        print(
            _(
                "Invalid output directory. If input is a directory then --output must also be set to a directory"
            )
        )
        sys.exit(1)
    if not (os.path.isfile(args.input) or os.path.isdir(args.input)):
        print(
            _("Invalid input. No file or directory at {input_path}").format(
    ...
    if args.temporary_directory and not os.path.isdir(args.temporary_directory):
        print(
            _("Temporary directory {temporary_path} doesn't exist. Creating…").format(
                temporary_path=args.temporary_directory
            )
        )
        os.makedirs(args.temporary_directory)

    if detection_modelfile := ModelFiles.get_detection_model_by_name(
        args.mosaic_detection_model
    ):
    ...
    if restoration_modelfile := ModelFiles.get_restoration_model_by_name(
    ...
    if args.encoder:
        ...
        print(
            _(
                'Either "--encoding-preset" or "--encoder" together with "--encoder-options" must be used'
            )
        )
        sys.exit(1)
    ...
    input_files, output_files = utils.setup_input_and_output_paths(
        args.input, args.output, args.output_file_pattern
    )
